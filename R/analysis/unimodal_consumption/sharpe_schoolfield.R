...
# Standardize to modelled peak temperature
# Loop
tmp_dat <- data.frame()
datalist <- list()
...
# Add in modelled peak temperature...
colnames(con)
...
# Center temperature now with modelled temp
...
model = "R/analysis/JAGS_models/unimodal_consumption/sharpe_school.txt"
...
p9 <- ggplot(pred_df, aes(temp, median)) +
  ...
  # geom_point(data = con, aes(temp_ct, y_ct, color = species_ab),
  #            size = 2, alpha = 0.8) +
  geom_point(data = con, aes(peak_temp_c_model_ct, y_ct, color = species_ab),
             size = 2, alpha = 0.8) +
  scale_color_manual(values = pal, name = "Species") +
  annotate("text", -27.5, 2.1, label = paste("n=", nrow(con), sep = ""), size = 4) +
...
ggsave("figures/SharpeSchool_scatter.png", width = 6.5, height = 6.5, dpi = 600)


####*************  TEST: Testing plotting it together with the conceptual figure ####
...
# Plot all 6 curves
pa <- dat %>% filter(y_g_d_temp > 0 & temp_c_ct > t_min & temp_c_ct < t_max) %>% 
...
pWorda <- pa + theme_classic() + theme(text = element_text(size = 12),
                                       aspect.ratio = 1,
                                       legend.title = element_text(size = 10),
...
pWord9 + pWorda
ggsave("figures/SharpeSchool_concept.png", width = 7, height = 7, dpi = 600)

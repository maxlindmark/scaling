...
#
# * WORK IN PROGRESS, MORE A TEST THAN ANYTHING ELSE *
# - Code to try and fit non-linear (polynomial) models to consumption 
# 
# A. Load libraries
#
# B. Read data
...
model_no_inter = "R/analysis/non-linear model/interaction.txt"
...
for(i in unique(con$species_ab)){
  ...
  jdat = list(
    ...
    mass = (log(df$mass_g) - mean(log(df$mass_g))),
  ...
  est_dat[[i]] <- ggs(cs) %>% filter(Parameter == "b4") %>% mutate(species = i) 
...
est_dat_df <- dplyr::bind_rows(est_dat)

str(pred_dat_df)

# Plot estimates of b4 (interaction parameter)
est_dat_df
...
# Standardize predictions, similar in Englund et al 2011
...
# Now standardize y wrt max predicted
...
pred_dat_df %>% 
  ...
  geom_vline(xintercept = sum_tot$mean_temp - sum_tot$stdev_temp, 
             linetype = 3, color = "gray20", size = 0.7) + 
  geom_vline(xintercept = sum_tot$mean_temp + sum_tot$stdev_temp, 
             linetype = 3, color = "gray20", size = 0.7) + 
  geom_point(data = sub2,  aes(temp_dat, y_stand, color = factor(species)),
...
pred_dat_df %>% 
  filter(median > 0) %>% 
  ggplot(., aes(temp, median_stand)) +
  facet_wrap(~ factor(species)) +
  geom_point(data = filter(sub2, y_stand > 0),  aes(temp_dat, y_stand),
             size = 3, alpha = 0.8, shape = 21) +
  geom_line(size = 1, alpha = 0.6) +
...
# left_join with miniale example
# I need to predict on a realistic mass AND temperature, needs to be species-specific.
# anyway, no need to have a single one for all
...
cons_1 <- 1 + 0.55*m1 + 0.05*temp - 0.003*temp*temp + 0.02*temp*m1
cons_2 <- 1 + 0.55*m2 + 0.05*temp - 0.003*temp*temp + 0.02*temp*m2

plot(cons_1 ~ temp, ylim = c(0.5, 2))
lines(cons_2 ~ temp, col = "red")

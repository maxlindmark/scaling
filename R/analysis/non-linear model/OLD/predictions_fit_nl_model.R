...
temp_pred_met = seq(from = min(con$temp_norm_ct), 
...
ggplot(met, aes(log_mass_norm_ct)) + 
  geom_histogram() +
  geom_vline(xintercept = c(-2.5, 2.5), color = "red")

ggplot(met, aes(log_mass_norm_ct, mass_norm)) + 
  geom_point() +
  geom_vline(xintercept = c(-2.5, 2.5), color = "red")
...
# First fit models. See "meta_nl_model_selection"
# Metabolism
